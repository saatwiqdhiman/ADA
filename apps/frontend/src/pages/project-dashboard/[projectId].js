...
const ProjectDashboard = () => {
  ...
  useEffect(() => {
    if (router.isReady) {  // Wait until the router is ready
      console.log("Mounted and router is ready");
      const {projectId} = router.query 
      if (projectId) {
        setProjectId(projectId);
        console.log('Router query projectid:', projectId); // Debugging projectId from the URL
        fetchProjectDetails(projectId);
      } else {
        console.log('No projectid found in the query');
      }
    }
  ...
  const fetchProjectDetails = async (id) => {
    try {
      ...
      const { data } = await api.get(`/api/projects/${id}`);  // Fetching project details
      console.log('API Response:', data);  // Debugging API response
  
      if (data && data.name) {
        setProjectName(data.name);  // Use data.name if it's a single project object
        console.log('Project Name:', data.name);  // Debugging the project name
      } else {
